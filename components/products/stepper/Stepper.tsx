...
export default function Stepper({ step, setStep, steps }: StepperProps) {
    ...
    return (
        <Tabs value={`step-${step}`} onValueChange={onValueChange} className="h-auto overflow-hidden relative">
            <TabsList className="bg-transparent flex justify-between h-auto! px-0 py-6 border-border border-t border-b w-full md:mx-4">
                {
            ...
            <div
                ...
                className="pointer-events-none flex justify-between h-auto! px-0 py-6 w-full absolute transition-[clip-path] duration-200 ease-in-out md:mx-4 md:duration-250"
            >
                {
            ...
            <div className={`flex transition ease-in-out duration-500`} style={{ width: `${steps.length}00%`, transform: `translateX(${-(step - 1) / steps.length * 100}%)` }}>
                {
                    steps.map(({ item }, id) => (
                        <div key={id} className="flex-1 px-4">
                            {item}
                        </div>
                    ))
                }
