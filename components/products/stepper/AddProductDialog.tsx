...
export default function AddProductDialog({ producents, categories, features }: AddProductDialogProps) {
    return (
        <Dialog>
            ...
            <DialogContent className="gap-0 transition-all duration-700 overflow-y-auto flex flex-col translate-0 data-open:animate-[animation-in_0.5s_ease-in-out] [&:not([data-open])]:animate-[animation-out_0.5s_ease-in-out] max-w-none rounded-none top-0 left-0 bottom-0 right-0 pt-6 pb-0 px-0 md:max-w-180 md:top-1/2 md:bottom-auto md:left-1/2 md:right-auto md:-translate-1/2 md:rounded-[14px] [&>button]:right-4 [&>button]:top-6 md:data-open:animate-[animation-in-2_0.5s_ease-in-out] md:[&:not([data-open])]:animate-[animation-out-2_0.1s_ease-in-out]">
